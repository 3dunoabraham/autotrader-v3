...
class makeTradeController {
  constructor($container, options = null, data = null)
  {
    ...
    this.form = {
      ...
      priceAdjust: document.getElementById('input-tradePriceAdjust'),
    ...
    this.updaters =
    {
      cardTrade(event = null)
      {
        ...
        self.formCard.balance.innerHTML = ((tradeResponse.data.quantity * tradeResponse.data.bidPrice) / self.data.leverageLevel).toFixed(2);
      ...
      refreshPriceButton(loading)
      {
        if (loading)
        {
          document.getElementById("refreshPrice-button").className = document.getElementById("refreshPrice-button").className.replace(" fa-check ", " fa-sync-alt fa-spin ");
        } else {
          document.getElementById("refreshPrice-button").className = document.getElementById("refreshPrice-button").className.replace("fa-sync-alt fa-spin", " fa-check ");
        }
      },
    };
    this.watchers =
    {
      ...
      priceAdjustChanged(event = null)
      {
        console.log(("priceAdjust"))
        console.log(self.form.priceAdjust)
        self.form.priceAdjust.value = event.currentTarget.value

        console.log("priceAdjustChanged")
      },
    ...
    this.clickers =
    {
      ...
      setPriceAdjust(event = null)
      {
        console.log("qwe")

        self.form.priceAdjust = parseInt(event.currentTarget.innerHTML);

        console.log(self.form.priceAdjust)

        self.watchers.priceAdjustChanged({
          currentTarget: {
            value: parseInt(event.currentTarget.innerHTML),
          },
        })
      ...
      setAmount(event = null)
      {
        // quantity.value
        // self.watchers.quantityChanged()
        self.form.quantity.value = parseInt(event.currentTarget.innerHTML)
        self.watchers.quantityChanged()
      },
...
let tradeEvent = (event) =>
{
  ...
  inputMsg.innerHTML += '<i class="fas demo-meta fa-8x fa-circle-notch  fa-check"></i>'
  ...
  newPriceRequest.onload = () =>
  {
    ...
    if (withPNLCheckbox.checked && parseFloat(data.pnl.profitPercent) != 0)
    {
      ...
      <div class="row between-xs">
        ...
        <span class="col-xs">+$${((data.pnl.profitPercent * self.data.leverageLevel) / 100) * quantity}</span>
        <span class="col-xs">+$${parseInt(data.pnl.profitPercent * self.data.leverageLevel)}%</span>
      </div>`;
    } else {
      tradeMsg.innerHTML += `
      <div class="row between-xs">
        ...
        // <span class="col-xs">${data.pnl.profitPercent * self.data.leverageLevel}% * ${quantity}</span>
    ...
    if (withPNLCheckbox.checked && parseFloat(data.pnl.lossPercent) != 0)
    {
      ...
      <div class="row between-xs">
        ...
        <span class="col-xs">-$${((data.pnl.lossPercent * self.data.leverageLevel) / 100) * quantity}</span>
        <span class="col-xs">+$${parseInt(data.pnl.lossPercent * self.data.leverageLevel)}%</span>
      </div>`;
    } else {
      tradeMsg.innerHTML += `
      <div class="row between-xs">
